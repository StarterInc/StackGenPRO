 ...
 * 
 * TODO: obviously use SSL? 
 * 
 * FIXME it shouldn't be
 * possibile to change serverhost/port: these should be changed at the same time
 * re-creating the hostConfig
 */
public class BaseCaldavClient extends HttpClient {
	...
	public BaseCaldavClient(Properties props) {
		this(props.get("CALDAV_SERVER_HOST").toString(), props.get(
				"CALDAV_SERVER_PORT").toString(), props.get(
				"CALDAV_SERVER_PROTOCOL").toString(), props.get(
				"CALDAV_SERVER_WEBDAV_ROOT").toString());
	}
	 ...
	 * @param webDavRoot
	 */
	public BaseCaldavClient(String host, String port, String protocol,
			String webDavRoot) {
	...
	protected void mkdir(String path) {
		...
		mk.setPath(path);
		try {
			executeMethod(hostConfig, (HttpMethod) mk);
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
