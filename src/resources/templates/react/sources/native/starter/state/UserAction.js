...
export function failedCreateUser(e) {
  if(typeof(e.response) === 'undefined'){
    ...
    showLoginFailed(strings.SimpleCMS_User_already_exists);
  } else if(e.response.status === 500){
    showLoginFailed(strings.SimpleCMS_User_unexpected_server_error);
  }
...
// json.User + ' : ' + JSON.stringify(json));
  ...
  if(GLOBAL.JSESSIONID === null){
    let error = new Error(strings.SimpleCMS_User_unexpected_server_error)
