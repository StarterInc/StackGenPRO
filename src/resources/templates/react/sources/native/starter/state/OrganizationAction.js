...
export function failedCreateOrganization(e) {
  if(typeof(e.response) === 'undefined'){
    ...
    showLoginFailed(strings.SimpleCMS_Organization_already_exists);
  } else if(e.response.status === 500){
    showLoginFailed(strings.SimpleCMS_Organization_unexpected_server_error);
  }
...
// json.Organization + ' : ' + JSON.stringify(json));
  ...
  if(GLOBAL.JSESSIONID === null){
    let error = new Error(strings.SimpleCMS_Organization_unexpected_server_error)
